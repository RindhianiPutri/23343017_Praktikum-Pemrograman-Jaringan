...
async function main() {
    try {
        ...
        // Menggunakan 'await' untuk menghubung ke server MongoDB.
        ...
        const c1Pengguna = db.collection('pengguna');

        // Memilih koleksi 'tugas' di dalam database.
        const c1Tugas = db.collection('tugas');

        // MEMASUKAN SATU DATA (DOKUMEN)
        // Memasukkan dokumen ke dalam koleksi 'pengguna'.
        const insertPengguna = await c1Pengguna.insertOne({
            id: id,
            nama: 'Rindhi',
            usia: 20
        });
        ...
        const insertTugas = await c1Tugas.insertMany([
            {
